...
class PostgreSQLAdapter:
        ...
        try:
            ...
            cursor.execute(f"""
                ...
                CREATE INDEX IF NOT EXISTS idx_audit_org_id
                    ON {self.config.schema}.{self.config.audit_table}(org_id);
                CREATE INDEX IF NOT EXISTS idx_audit_workspace_id
                    ON {self.config.schema}.{self.config.audit_table}(workspace_id);
            """)
    ...
    def save_audit_log(self, audit_log: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Extract fields (including multi-tenant context)
            metadata = audit_log.get("metadata", {})
            org_id = metadata.get("org_id") if isinstance(metadata.get("org_id"), str) else None
            workspace_id = metadata.get("workspace_id") if isinstance(metadata.get("workspace_id"), str) else None
            ...
            cursor.execute(f"""
                INSERT INTO {self.config.schema}.{self.config.audit_table} (
                    ...
                    %(timestamp)s, %(session_id)s, %(request_id)s,
                    %(org_id)s, %(workspace_id)s,
                    %(user_id)s, %(user_role)s, %(ip_address)s,
    ...
    def get_audit_logs(
        ...
        """
        Query audit logs (multi-tenant aware)
        ...
        Args:
            ...
            org_id: Filter by organization ID (multi-tenant)
            workspace_id: Filter by workspace ID (multi-tenant)
        ...
        try:
            ...
            if workspace_id:
                query += " AND workspace_id = %(workspace_id)s"
    ...
    def get_compliance_stats(
        ...
        """
        Get compliance statistics for a time period (multi-tenant aware)

        Args:
            start_date: Start of period
            end_date: End of period
            org_id: Filter by organization ID (required for multi-tenant)
